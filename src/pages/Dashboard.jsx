...
const KPI_CARDS = [
  { id: 'total-revenue', label: 'Total Revenue', value: '₹45,231', change: '+12.5%', trend: 'up', icon: DollarSign, color: 'from-blue-500 to-blue-600', shadow: 'shadow-blue-500/20' },
  { id: 'active-clients', label: 'Active Clients', value: '1,234', change: '+3.2%', trend: 'up', icon: Users, color: 'from-purple-500 to-pink-500', shadow: 'shadow-purple-500/20' },
  { id: 'total-streams', label: 'Total Streams', value: '8,456', change: '-2.4%', trend: 'down', icon: Activity, color: 'from-orange-500 to-red-500', shadow: 'shadow-orange-500/20' },
  { id: 'growth-rate', label: 'Monthly Growth', value: '23.5%', change: '+4.1%', trend: 'up', icon: TrendingUp, color: 'from-emerald-500 to-teal-500', shadow: 'shadow-emerald-500/20' },
];
...
const Dashboard = () => {
  return (
    <div className="space-y-8">
      <PageHeader title="Dashboard Overview" subtitle="Real-time store performance insights">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {KPI_CARDS.map((kpi) => <KpiCard key={kpi.id} kpi={kpi} />)}
      </div>
          ...
          <div className="h-[400px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={REVENUE_DATA} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                ...
                <Tooltip contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 10px 15px -3px rgba(0, 0, 0, 0.1)' }} itemStyle={{ fontSize: '14px', fontWeight: 600 }} />
                <Area type="monotone" dataKey="revenue" stroke="#2563eb" strokeWidth={3} fillOpacity={1} fill="url(#colorRevenue)" activeDot={{ r: 8, fill: '#2563eb', strokeWidth: 0 }} />
                <Area type="monotone" dataKey="organic" stroke="#10b981" strokeWidth={3} fillOpacity={1} fill="url(#colorOrganic)" activeDot={{ r: 6, fill: '#10b981', strokeWidth: 0 }} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </Card>

        <div className="lg:col-span-1 bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl shadow-xl border border-white/10 p-6 sm:p-8 text-white relative overflow-hidden">
          <div className="absolute top-0 right-0 w-48 h-48 bg-blue-500/10 rounded-full blur-3xl -translate-y-10 translate-x-10" />
          <div className="absolute bottom-0 left-0 w-32 h-32 bg-purple-500/10 rounded-full blur-2xl translate-y-5 -translate-x-5" />

          <h2 className="text-xl font-bold tracking-tight mb-6 relative z-10">Quick Stats</h2>

          <div className="space-y-4 relative z-10">
            {[
              { label: 'Active Sessions', value: '24' },
              { label: "Today's Revenue", value: '₹12,450' },
              { label: 'New Clients', value: '8' },
              { label: 'Avg. Session Time', value: '45 min' },
            ].map((stat) => (
